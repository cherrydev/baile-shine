# Implement the neopixel protocol on an i2s bus

# The data is stored expanded into the format that will be sent
# directly to the i2s bus. This is efficient if there are realtively
...
# _expanded_bits = [0x88, 0x8e, 0xe8, 0xee]
# _expanded_bits = [0x88, 0x8e, 0xe8, 0xee]
_expanded_bits = [0x88, 0x8C, 0xC8, 0xCC]
# _expanded_bits = [0x8C, 0x88,0xCC, 0xC8]
_expanded_bits_bytes = bytes((0x88, 0x8C, 0xC8, 0xCC))

@micropython.viper
def _expand_byte_vip(b: int, mv_o):
    mv = ptr8(mv_o)
    expanded_bits = ptr8(_expanded_bits_bytes)
    mv[1] = expanded_bits[(b >> 6) & 0x3]
    mv[0] = expanded_bits[(b >> 4) & 0x3]
    mv[3] = expanded_bits[(b >> 2) & 0x3]
    mv[2] = expanded_bits[(b) & 0x3]

# @micropython.native
def _expand_byte(b: int, mv):
    mv[1] = _expanded_bits[(b >> 6) & 0x3]
    mv[0] = _expanded_bits[(b >> 4) & 0x3]
    mv[3] = _expanded_bits[(b >> 2) & 0x3]
    mv[2] = _expanded_bits[(b) & 0x3]
...
class NeoPixel:
    ...
    def __init__(self, i2s_bus: int, i2s_pin : I2S, pixel_count: int, *, rate=92_000):
        self._n = pixel_count
        self._ibuf_len = max(pixel_count * 12, 2048)
        self._ebuf_len = pixel_count * 12
        self._data = memoryview(bytearray(self._ebuf_len))
        self._zero_buf = memoryview(bytearray(300))
        self._spy = Pin(13, Pin.OUT)
        self._init = False
        self._i2s : I2S = None
        def i2s_init() :
            if not self._init:
                self._init = True
                self._i2s = I2S(i2s_bus,
                    sd = i2s_pin,
                    sck = Pin(26),
                    ws = Pin(27),
                    mode=I2S.TX,
                    ...
                    format = I2S.STEREO,
                    rate = rate,
                    ibuf = self._ibuf_len * 2
                )
        i2s_init()

        self._i2s_init = i2s_init
        self._spy.off()
        self[:] = (0,0,0)

    
    def _i2s_deinit(self):
        if self._init:
            self._init = False
            self._i2s.deinit()
    ...
    def write(self):
        ...
        self._spy.on()
        self._i2s.irq(None)
        self._i2s.write(self._data)
        self._i2s.write(bytearray(300))
        # self._i2s.write(bytearray(300))
        self._spy.off()

    async def write_nonblock(self):
        self._i2s_init()
        lock = uasyncio.ThreadSafeFlag()
        self._i2s.irq(lambda v: lock.set())
        self._spy.on()
        self._i2s.write(self._data)
        await lock.wait()
    ...
    async def write_async(self):
        self._i2s_init()
        swriter = uasyncio.StreamWriter(self._i2s)
        self._spy.on()
        swriter.write(self._data)
        await swriter.drain()
        await uasyncio.sleep_ms(4 + 5)
        self._i2s_deinit()
        swriter.close()
        self._spy.off()

    async def write_async2(self):
        swriter = uasyncio.StreamWriter(self._i2s)
        self._spy.on()
        swriter.write(self._data)
        swriter.write(self._zero_buf)
        await swriter.drain()
        swriter.close()
        self._spy.off()
